...
| ID | Task | Owner | Status | Notes |
|---|---|---|---|---|
| T-109 | The dock publishes its own box as `--dock-w` / `--dock-h` (one `ResizeObserver`, no React state); above 640 px `.poi-strip` and `.corner` cap at `(100% − var(--dock-w)) / 2 − 16px`, and while a category is browsed (`.dock-actives`) the whole bottom bar lifts above the dock (`bottom: calc(var(--dock-h) + 28px)`) so the attribution is never covered. Correct by construction because the dock is centred on the bar in both chromes | map-ui-dev → review → qa | done | Alternatives weighed and rejected in the PR: a summary chip past N, a static max-width (no constant works — the agent lane moves the gutter), one flex row (touches the tray anchor, landing hint and four media queries). Review: merge after fixes (stale comment, duplicate `.badge` rule, `usePublishedBox` contract) — applied. Layout is not measurable in the node vitest environment, so the gate is e2e: `bottom-band.spec.ts` sweeps 18 cells (921/1024/1440 × human/agent chrome × none/3 browsed/15 loaded) with `boundingBox()` intersections and `elementFromPoint` on every attribution link; 13/18 red on develop, 54/54 green on the fix (`--repeat-each=3`). ≤640 px stays F-14 |
...
- **F-16 (qa)** ~~`e2e/awakening-flight.spec.ts:249` reads bounds immediately after `setViewportSize` and races the republish (~4/24 on develop).~~ Fixed 2026-09-02, rides the T-109 PR: the spec captures the raw bounds tuple before the resize and `waitForFunction`s until it changes (bounded 1,500 ms, no sleep); 48/48 with `--repeat-each=12`.
- **F-17 (map-ui-dev)** With the inspector open at 1350 px, the *open* Places tray's bottom-right corner overlaps the OpenFreeMap attribution pill by ~7 CSS px (tray bottom 835 vs attribution top 828). The tray is anchored above the dock and the attribution to the map's right edge; T-109's caps and lift only govern the closed bar, not the tray panel. Cosmetic; the links stay clickable. Found capturing Devpost media 2026-09-02.
...
- 2026-09-02 · qa → all (lesson) · Any real tool call wakes the page into agent chrome, so a human-chrome e2e cell must reach its content state by hand: `bottom-band.spec.ts` loads 15 categories by tapping chips in the Places tray (a 4th tap evicts the oldest *painted* chip but never unloads it) and un-paints with the chips' `×`. Also: `page.waitForFunction(fn, arg, options)` — the timeout goes in the third argument; in the second it is silently the page function's argument and the wait falls back to the test timeout.
