...
import { zoomToFit } from "./mercator";

/**
 * Where the camera goes when something on this map is asked to be shown.
 *
 * Two callers, one answer. A person clicks a row in the inspector
 * (`components/Inspector.tsx` via `components/frame-model.ts`, T-101); an agent
 * calls `set_map_view({fit})` (T-102). Both mean *show me that one*, and both
 * get their `{ center, zoom }` from here — a human's click and an agent's call
 * must land the camera in the same place, and the only way to promise that is
 * for the two paths to be one function. The maths moved into `lib/` for T-102
 * unchanged; `components/frame-model.ts` re-exports it, so the human path did
 * not notice.
 *
 * **The row click is still the human acting, and nothing here changes that.**
 * That caller writes `setView`, which is the same write a pan makes; no tool is
 * called, no activity row is recorded, and the awakening (`lib/awaken`, which
 * reads `activity` and `restoredAgentState`) never hears about it. The camera
 * chip and the share hash both read `view` from the store, so they follow for
 * free.
 */
 ...
 * the tool registry. That `set_map_view({fit})` on a point behaves exactly like
 * `set_map_view({feature_id})` is a promise its description makes and its test
 * holds — not something either constant knows about the other.
 *
 * The `ROW_` names are historical (the inspector row click was the first
 * caller) and kept so T-101's call sites and tests read unchanged.
 */
 ...
 * little as possible. (`set_map_view({fit})` never reaches that branch: it
 * refuses with "map not ready" instead, because an agent that cannot see the
 * screen would read a silent point-rule fallback as a fit.)
 */
